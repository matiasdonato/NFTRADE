...
export default async function emailCollection(
  req: NextApiRequest,
  res: NextApiResponse,
  userId: string,
  collectionId: string,
  ...
  if (reason === 'comprador') {
    const info = await prisma.collection.findUnique({
    ...
    const mailOptions = {
      ...
      subject: 'Collection Buy',
      html: `
      <p> ${date} </p>
      <h1>THE COLLECTION WAS SUCCESFULLY BUYED</h1>
      <div style="background-color:#e5e7eb; width: 40%; text-align: center;"> 
        ...
        <img width="300px" height="300px" src=${info?.image}  alt="collection"/>
      </div>
